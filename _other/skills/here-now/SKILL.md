...
- Required: Python 3 and [rich](https://github.com/Textualize/rich) (`uv run --with rich …`)
...
- Bundled helpers:
  - `./scripts/publish.py` for publishing sites
  - `./scripts/drive.py` for private Drive storage

## Create a site

```bash
./scripts/publish.py {file-or-dir}
```
...
```bash
./scripts/publish.py {file-or-dir} --slug {slug}
```
...
```bash
./scripts/drive.py default
./scripts/drive.py ls My Drive
./scripts/drive.py put My Drive notes/today.md --from ./notes/today.md
./scripts/drive.py cat My Drive notes/today.md
./scripts/drive.py share My Drive --perms write --prefix notes/ --ttl 7d
```

Use scoped Drive tokens for agent-to-agent handoff. If you receive a `herenow_drive` share block, use its `token` as `Authorization: Bearer <token>` against `api_base`, respect `pathPrefix` when present, and preserve ETags on writes. A `pathPrefix` of `null` means full-Drive access. If the skill is available, prefer `./scripts/drive.py`; otherwise call the listed API operations directly.
...
```bash
./scripts/publish.py {file-or-dir} --client cursor
```

This sends `X-HereNow-Client: cursor/publish-py` on publish API calls.
...
For Drive operations, use `./scripts/drive.py` or the Drive API. For broader account and Site management — Site Data, search, analytics, profiles, delete, metadata, access control, domains, subdomain handles, links, variables, proxy routes, duplication, and more — see the current docs:
